...
const db = new Database(path.join(dataDir, 'centralcuryvendas.db'));
...
['ALTER TABLE users ADD COLUMN role TEXT NOT NULL DEFAULT \'admin\'',
 ...
 'ALTER TABLE leads ADD COLUMN phone_raw TEXT NOT NULL DEFAULT \'\'',
...
const DEFAULT_CONFIG = {
  ...
  email_smtp_user:  '',
  email_smtp_pass:  '',
  email_from:       'Central Cury Vendas <noreply@centralcuryvendas.com.br>',
  email_to:         '',
...
// ---- In-memory real-time visitors (session_id → {page, last_seen}) ----
const activeVisitors = new Map();
const VISITOR_TIMEOUT_MS = 5 * 60 * 1000; // 5 min

function cleanVisitors() {
  const cutoff = Date.now() - VISITOR_TIMEOUT_MS;
  for (const [sid, v] of activeVisitors) {
    if (v.last_seen < cutoff) activeVisitors.delete(sid);
  }
}

// ---- Deduplicação de leads ----
// Normaliza telefone: remove não-dígitos, DDI (55/0055) e zero de DDD (021→21)
...
// Busca lead existente por telefone (dígitos) ou email
function findDuplicateLead(phone, email) {
  const digits = normalizePhone(phone);
  if (digits) {
    // Compara dígitos de ambos os lados via SQLite (remove não-dígitos no campo)
  ...
  if (email && email.trim()) {
    const row = db.prepare(`SELECT * FROM leads WHERE email != '' AND lower(email) = lower(?) ORDER BY id ASC LIMIT 1`).get(email.trim());
    if (row) return row;
  }
  return null;
}

// Acrescenta uma entrada de atividade nas notes do lead
function appendActivity(leadId, label, extra) {
  const existing = db.prepare('SELECT notes, email, interest FROM leads WHERE id = ?').get(leadId);
  if (!existing) return;
  const ts = new Date().toLocaleString('pt-BR', { timeZone: 'America/Sao_Paulo' });
  const line = `[${ts}] ${label}${extra ? ': ' + extra : ''}`;
  const notes = existing.notes ? existing.notes + '\n' + line : line;
  db.prepare('UPDATE leads SET notes = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?').run(notes, leadId);
}

// Mescla campos novos no lead duplicado (preenche vazios, atualiza interest)
function mergeDuplicate(existing, { email, interest, message, source }) {
  const updates = [];
  const params = [];
  if (email && email.trim() && !existing.email) { updates.push('email = ?'); params.push(email.trim()); }
  if (interest && interest.trim()) { updates.push('interest = ?'); params.push(interest.trim()); }
  if (updates.length) {
  ...
  if (interest && interest.trim()) parts.push(`interesse: ${INTEREST_LABELS[interest] || interest}`);
  if (message && message.trim()) parts.push(`msg: "${message.trim().slice(0, 120)}"`);
  const label = source === 'whatsapp' ? 'Novo contato via WhatsApp' : 'Novo contato via formulário';
  appendActivity(existing.id, label, parts.join(' | '));
}
...
async function notifyEmail(lead, cfg) {
  ...
  await transport.sendMail({
    from:    cfg.email_from || 'Central Cury Vendas <noreply@centralcuryvendas.com.br>',
    to:      cfg.email_to,
    subject: `🏠 Novo Lead: ${lead.name} – Central Cury Vendas`,
    html: `
<!DOCTYPE html><html lang="pt-BR"><body style="margin:0;padding:0;background:#f4f4f4;font-family:Arial,sans-serif">
<div style="max-width:600px;margin:24px auto">
  <div style="background:#7b1c2e;padding:28px 32px;border-radius:10px 10px 0 0">
    <h1 style="color:#fff;margin:0;font-size:22px">🔔 Novo Lead — Central Cury Vendas</h1>
  </div>
  <div style="background:#fff;padding:32px;border-radius:0 0 10px 10px;border:1px solid #e5e5e5">
    ...
    <div style="margin-top:28px;text-align:center">
      ...
      <a href="tel:${lead.phone}" style="background:#7b1c2e;color:#fff;padding:13px 28px;border-radius:6px;text-decoration:none;font-weight:bold;display:inline-block">Ligar</a>
    </div>
  </div>
  <p style="text-align:center;color:#bbb;font-size:12px;margin-top:16px">Central Cury Vendas · Notificação automática</p>
</div></body></html>`,
  });
}

async function notifyEvolution(lead, cfg, attendantPhone = null) {
  if (cfg.evolution_enabled !== 'true') return;
  if (!cfg.evolution_url || !cfg.evolution_instance || !cfg.evolution_apikey) return;
  ...
  // Monta lista de destinatários: número principal + atendente da vez (sem duplicar)
  const mainPhone  = (cfg.evolution_phone || '').replace(/\D/g, '');
  const attPhone   = (attendantPhone || '').replace(/\D/g, '');
  const recipients = [...new Set([mainPhone, attPhone].filter(Boolean))];
  ...
  for (const number of recipients) {
    ...
    if (!res.ok) {
      ...
      throw new Error(`Evolution API [${number}]: HTTP ${res.status} ${detail}`);
    }
  }
}
...
// attendant: objeto { id, name, phone } ou null
function fireNotifications(lead, cfg, attendant = null) {
  const attPhone = attendant?.phone || null;
  Promise.allSettled([
    notifyEmail(lead, cfg),
    notifyEvolution(lead, cfg, attPhone),
...
app.post('/api/analytics/pageview', rateLimit(60 * 1000, 30), (req, res) => {
  const { session_id, page = '/', referrer = '', utm_source = '', utm_medium = '',
    utm_campaign = '', utm_content = '', utm_term = '', device = 'desktop' } = req.body;
  if (!session_id) return res.json({ ok: true });

  db.prepare(`
    INSERT INTO pageviews (session_id, page, referrer, utm_source, utm_medium, utm_campaign, utm_content, utm_term, device)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(session_id, String(page).slice(0,200), String(referrer).slice(0,500),
    String(utm_source).slice(0,100), String(utm_medium).slice(0,100),
    String(utm_campaign).slice(0,100), String(utm_content).slice(0,100),
    String(utm_term).slice(0,100), String(device).slice(0,20));

  activeVisitors.set(session_id, { page: String(page).slice(0,200), last_seen: Date.now() });
  res.json({ ok: true });
});

// ---- Analytics: heartbeat de visitante ativo ----
...
app.post('/api/leads', rateLimit(5 * 60 * 1000, 10), (req, res) => {
  const { name, phone, phone_raw = '', email = '', interest = '', message = '' } = req.body;
  ...
  if (existing) {
    mergeDuplicate(existing, { email, interest, message, source: 'landing_page' });
    const lead = db.prepare('SELECT * FROM leads WHERE id = ?').get(existing.id);
    const attendant = existing.attendant_id
      ? db.prepare('SELECT * FROM attendants WHERE id = ?').get(existing.attendant_id)
      : null;
    fireNotifications(lead, cfg, attendant);
    return res.json({ success: true, id: lead.id, duplicate: true });
  }

  const attendant = nextAttendant('form_queue_idx');
  const r = db.prepare(`
    INSERT INTO leads (name, phone, phone_raw, email, interest, message, source, attendant_id)
    VALUES (?, ?, ?, ?, ?, ?, 'landing_page', ?)
  `).run(name.trim(), phone.trim(), (phone_raw || phone).trim(),
    email.trim(), interest.trim(), message.trim(), attendant?.id || null);
...
// Receber lead do WhatsApp (fila WA) — retorna URL do atendente da vez
app.post('/api/leads/wa', rateLimit(5 * 60 * 1000, 10), (req, res) => {
  const { name, phone, phone_raw = '' } = req.body;
  if (!name?.trim() || !phone?.trim()) {
    return res.status(400).json({ error: 'Nome e telefone são obrigatórios.' });
  }

  const cfg = getConfig();
  const existing = findDuplicateLead(phone, null);

  if (existing) {
    mergeDuplicate(existing, { source: 'whatsapp' });
    const lead = db.prepare('SELECT * FROM leads WHERE id = ?').get(existing.id);
    const attendant = existing.attendant_id
      ? db.prepare('SELECT * FROM attendants WHERE id = ?').get(existing.attendant_id)
      : null;
    ...
    return res.json({ success: true, id: lead.id, wa_url: `https://wa.me/${waPhone}?text=${waMsg}`, duplicate: true });
  }

  const attendant = nextAttendant('wa_queue_idx');
  const r = db.prepare(`
    INSERT INTO leads (name, phone, phone_raw, source, attendant_id)
    VALUES (?, ?, ?, 'whatsapp', ?)
  `).run(name.trim(), phone.trim(), (phone_raw || phone).trim(), attendant?.id || null);
...
app.get('/api/leads/export-excel', auth, async (req, res) => {
  ...
  wb.creator = 'Central Cury Vendas CRM';
  wb.created = new Date();
  ...
  titleCell.value = `Central Cury Vendas – Relatório de Leads   (${new Date().toLocaleDateString('pt-BR')})`;
  titleCell.font = { name: 'Calibri', size: 16, bold: true, color: { argb: 'FFFFFFFF' } };
  titleCell.alignment = { horizontal: 'center', vertical: 'middle' };
  titleCell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF4A0F1B' } };
  ...
  subCell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF7B1C2E' } };
  ...
  const headers = [
    ...
    { header: 'Tel. digitado',  key: 'phone_raw',    width: 20 },
  ...
  headers.forEach((h, i) => {
    ...
    cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF4A0F1B' } };
    cell.alignment = { horizontal: 'center', vertical: 'middle', wrapText: false };
    cell.border = {
      bottom: { style: 'medium', color: { argb: 'FFBFD1E8' } },
      right:  { style: 'thin',   color: { argb: 'FF7B1C2E' } },
    };
  });
  headerRow.height = 28;
  ...
  leads.forEach((l, idx) => {
    ...
    // Badge colorido para status (coluna 7 após adição de phone_raw)
  ...
  footerCell.value = 'Central Cury Vendas CRM  •  Relatório gerado automaticamente';
  ...
  const filename = `leads_centralcuryvendas_${new Date().toISOString().slice(0,10)}.xlsx`;
...
const FAKE_LEAD = {
  ...
  email: 'teste@centralcuryvendas.com.br', interest: 'luzes-do-rio',
...
app.get('/api/analytics', auth, (req, res) => {
  ...
  // Real-time visitors
  const realtime = activeVisitors.size;
  const realtimePages = [];
  const pageCount = {};
  for (const [, v] of activeVisitors) {
    pageCount[v.page] = (pageCount[v.page] || 0) + 1;
  }
  for (const [page, count] of Object.entries(pageCount)) {
    realtimePages.push({ page, count });
  }
  realtimePages.sort((a, b) => b.count - a.count);

  // Visitantes por dia (pageviews únicos por session_id por dia)
  ...
  const leadsByDay = db.prepare(`
    SELECT date(created_at, 'localtime') as day, COUNT(*) as leads
    FROM leads
    WHERE created_at >= datetime('now', '-${days} days')
    GROUP BY day ORDER BY day
  `).all();

  // Fontes de tráfego (utm_source ou domínio do referrer)
  ...
  // Meios (utm_medium)
  const mediumRows = db.prepare(`
    SELECT COALESCE(NULLIF(utm_medium,''), 'none') as medium,
           COUNT(DISTINCT session_id) as visitors
    FROM pageviews
    WHERE created_at >= datetime('now', '-${days} days')
    GROUP BY medium ORDER BY visitors DESC LIMIT 10
  `).all();
  ...
  const campaignRows = db.prepare(`
    ...
    GROUP BY campaign ORDER BY visitors DESC LIMIT 20
  `).all();
  ...
  const topPages = db.prepare(`
    ...
    FROM pageviews
    WHERE created_at >= datetime('now', '-${days} days')
    GROUP BY page ORDER BY pageviews DESC LIMIT 15
  `).all();

  // Leads por empreendimento
  const leadsByInterest = db.prepare(`
    SELECT interest, COUNT(*) as leads
    FROM leads
    WHERE created_at >= datetime('now', '-${days} days')
    GROUP BY interest ORDER BY leads DESC
  `).all();
  ...
  const totalLeads = db.prepare(`
    SELECT COUNT(*) as n FROM leads WHERE created_at >= datetime('now', '-${days} days')
  `).get().n;

  // Taxa de conversão
  const convRate = totals.total_visitors > 0
    ? ((totalLeads / totals.total_visitors) * 100).toFixed(1)
    : '0.0';

  res.json({
    realtime, realtimePages,
    totals: { ...totals, total_leads: totalLeads, conv_rate: convRate },
    visitorsByDay, leadsByDay,
    sources: sourceRows, mediums: mediumRows,
    campaigns: campaignRows, devices: deviceRows,
    topPages, leadsByInterest,
  });
});
...
// ============================================================
app.listen(PORT, () => {
  console.log(`\n✅ Central Cury Vendas rodando em http://localhost:${PORT}`);
